...
?>
		...
		<tbody>
			<tr class="thead">
				<?php
					...
					if(isset($_GET["ON"])) echo "<td width='20'>Мест</td>";
	...
	$query = "
		SELECT OD.OD_ID
			...
			,GROUP_CONCAT(IF(ODS.ODD_ID, CONCAT(IF(ODS.IsReady, CONCAT('<b>', IFNULL(ST.Short, 'Этап'), '</b>'), IFNULL(ST.Short, 'Этап')), '(<i>', IFNULL(USR_ShortName(ODS.USR_ID), '---'), '</i>)'), '') ORDER BY ST.Sort SEPARATOR '<br>') Steps
	...
	while( $row = mysqli_fetch_array($res) ) {
		...
		if(isset($_GET["ON"])) echo "<td width='20' style='font-size: 20px; text-align: center;'>{$row["boxes"]}</td>";
		if(isset($_GET["Z"])) {
//			$zakaz = "";
//			$options = "";
