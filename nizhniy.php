...
<body style='background: <?= ( $archive == 1 ) ? "#bf8" : "#fff" ?>'>
	...
	<p>
		...
		if( $archive == 1 )
		{
			echo "<a href='/nizhniy.php' class='button'>К в работе</a>";
		}
		else
		{
			echo "<a href='/nizhniy.php?archive=1' class='button'>К готовым</a>";
		}
		?>
	</p>
...
<?
	...
	$query = "SELECT OD.OD_ID
			  ...
			  LEFT JOIN (SELECT ODD.OD_ID
							   ...
							   ,CONCAT('<span', IF(IFNULL(ODD.Comment, '') <> '', CONCAT(' title=\'', ODD.Comment, '\''), ''), '>', IF(IFNULL(ODD.Comment, '') <> '', CONCAT('<i class=\'fa fa-comment\' aria-hidden=\'true\'></i>'), ''), ' ', ODD.Amount, ' ', IFNULL(PM.Model, '***'), ' ', IFNULL(CONCAT(ODD.Length, 'х', ODD.Width, IFNULL(CONCAT('/', ODD.PieceAmount, 'x', ODD.PieceSize), '')), ''), ' ', IFNULL(PF.Form, ''), ' ', IFNULL(PME.Mechanism, ''), ' ', '</span><br>') Zakaz
