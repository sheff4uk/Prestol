...
<?
	$query = "
		SELECT PVC.PVC_ID
			,PVC.edge
			,SUM(ODD.Amount) cnt
			,IFNULL(PVCL.cnt, 0) - SUM(IF(ODS.IsReady = 1, ROUND((IF(ODD.Width IS NULL, ODD.Length*PI(), (ODD.Length+ODD.Width)*2) + IFNULL(ODD.PieceSize, 0)*IFNULL(ODD.PieceAmount, 1)*2)/1000, 1), 0)) balance
			,SUM(IF(ODS.IsReady = 0, ROUND((IF(ODD.Width IS NULL, ODD.Length*PI(), (ODD.Length+ODD.Width)*2) + IFNULL(ODD.PieceSize, 0)*IFNULL(ODD.PieceAmount, 1)*2)/1000, 1), 0)) need
		FROM PVCedge PVC
