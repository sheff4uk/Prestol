<?
	...
	$query = "
		SELECT OD.OD_ID
			...
			,ODD.Price
			,ODD.discount
			,CONCAT('[', OD.Code, '] ', ODD.Amount, ' ', ODD.Zakaz ) Label
		FROM OrdersData OD
	...
	while( $row = mysqli_fetch_array($res) ) {
		$Products[] = array( "id"=>$row["ODDID"], "label"=>$row["Label"], "value"=>$row["Zakaz"], "Amount"=>$row["Amount"], "min_price"=>$row["min_price"], "Price"=>$row["Price"], "discount"=>$row["discount"], "odid"=>$row["OD_ID"], "code"=>$row["Code"] );
	}
