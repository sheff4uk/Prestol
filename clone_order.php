<?
	...
	if( isset($_GET["id"]) ) {
		...
		$query = "INSERT INTO OrdersData(CLientName, AddDate, StartDate, EndDate, SH_ID, OrderNumber, Color, Comment, creator)
				  SELECT CLientName, '{$AddDate}', StartDate, EndDate, SH_ID, OrderNumber, Color, Comment, {$_SESSION['id']}
				  FROM OrdersData
				  WHERE OD_ID = {$_GET["id"]}";
		...
		$query = "INSERT INTO OrdersDataDetail(OD_ID, PM_ID, Length, Width, PieceAmount, PieceSize, PF_ID, PME_ID, MT_ID, IsExist, Amount, Price, Comment, order_date, arrival_date, creator)
				  SELECT {$id}, PM_ID, Length, Width, PieceAmount, PieceSize, PF_ID, PME_ID, MT_ID, IsExist, Amount, Price, Comment, order_date, arrival_date, {$_SESSION['id']}
		...
		$query = "INSERT INTO OrdersDataBlank(OD_ID, BL_ID, Other, Amount, Price, Comment, MT_ID, IsExist, order_date, arrival_date, creator)
				  SELECT {$id}, BL_ID, Other, Amount, Price, Comment, MT_ID, IsExist, order_date, arrival_date, {$_SESSION['id']}
